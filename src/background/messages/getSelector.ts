...
const handler: PlasmoMessaging.MessageHandler<{ domain: string }, { selector: Selector }> = async (
  req,
  res
) => {
  const allRules: Rule[] = await storage.get(ExtensionStorage.UserRule)

  const validRules = allRules.filter((rule) => rule.domain === req.body!.domain && rule.enabled)
